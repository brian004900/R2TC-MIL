...
def one_fold(args,k,ckc_metric,train_p, train_l, test_p, test_l,val_p,val_l):
    ...
    # build network (RRTMIL + CRR cluster reweighting only)
    if args.model != 'rrtmil':
        raise ValueError(f"unsupported model={args.model}; only rrtmil is kept in R2TC-MIL")
    model_params = {
        ...
        'persistent_center': args.persistent_center,
        'nor_index': args.nor_index,
    }
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_best_model_stage', default=0., type=float, help='Fraction of epochs before saving best')
    ...
    # Model
    parser.add_argument('--model', default='rrtmil', type=str, help='Model name (only rrtmil)')
    ...
    # CRR (cluster re-weighting before bag classification)
    parser.add_argument('--num_cluster', default=3, type=int, help='CRR cluster count K')
    parser.add_argument('--cluster_distance', default='euclidean', type=str, help='CRR kmeans distance')
    parser.add_argument('--persistent_center', action='store_false', help='disable persistent CRR centers')
    parser.add_argument('--nor_index', default=0, type=int, help='CRR normal-class index for distress score')
