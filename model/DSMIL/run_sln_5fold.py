#!/usr/bin/env python3
"""DSMIL on SLN using official dsmil.MILNet + FCLayer (precomputed features).

Embed: ImageNet ResNet50-trunc 1024-d (this method's feats/), frozen.
Splits: /resnet 5-fold (no reshuffle).
"""

from __future__ import annotations

import argparse
import sys
from pathlib import Path

import numpy as np
import torch
import torch.nn as nn
from torch.utils.data import DataLoader

ROOT = Path(__file__).resolve().parent
sys.path.insert(0, str(ROOT))
sys.path.insert(0, str(ROOT.parents[0] / "_common"))
from dsmil import BClassifier, FCLayer, MILNet  # noqa: E402
from mil_utils import (  # noqa: E402
    SEED,
    BagFeatDataset,
    load_fold,
    metrics_from_scores,
    save_cv_summary,
    set_seed,
    write_epoch_txt,
)


def build_model(feats_size=1024, num_classes=1):
    i_classifier = FCLayer(feats_size, num_classes)
    b_classifier = BClassifier(input_size=feats_size, output_class=num_classes)
    return MILNet(i_classifier, b_classifier)


@torch.no_grad()
def eval_loader(model, loader, device):
    model.eval()
    ys, ps = [], []
    for feats, y, _ in loader:
        feats = feats.squeeze(0).to(device)
        classes, bag_pred, *_ = model(feats)
        # binary: bag logit → prob
        prob = torch.sigmoid(bag_pred).view(-1)[0].item()
        ys.append(int(y.item() if torch.is_tensor(y) else y))
        ps.append(float(prob))
    return metrics_from_scores(ys, ps)


def train_fold(fold, feat_dir, result_dir, epochs, lr, device):
    splits = load_fold(fold)
    loaders = {
        s: DataLoader(BagFeatDataset(splits[s], feat_dir, max_patches=4096, train=(s=="train")), batch_size=1, shuffle=(s == "train"))
        for s in ("train", "val", "test")
    }
    set_seed(SEED + fold)
    model = build_model().to(device)
    crit = nn.BCEWithLogitsLoss()
    opt = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=1e-3)

    best_val, best_ep, history = -1.0, -1, []
    fold_dir = result_dir / f"fold_{fold}"
    for epoch in range(1, epochs + 1):
        model.train()
        for feats, y, _ in loaders["train"]:
            feats = feats.squeeze(0).to(device)
            label = torch.tensor([[float(y.item())]], device=device)
            opt.zero_grad(set_to_none=True)
            classes, bag_pred, *_ = model(feats)
            # instance max + bag loss (DSMIL style)
            max_pred, _ = torch.max(classes, 0)
            loss_bag = crit(bag_pred.view(1, -1), label)
            loss_max = crit(max_pred.view(1, -1), label)
            loss = 0.5 * loss_bag + 0.5 * loss_max
            loss.backward()
            opt.step()
        metrics = {
            "train": eval_loader(model, loaders["train"], device),
            "val": eval_loader(model, loaders["val"], device),
        }
        write_epoch_txt(fold_dir / f"epoch_{epoch:03d}.txt", "DSMIL", fold, epoch, metrics)
        print(
            f"[DSMIL fold{fold}] ep{epoch:03d} "
            f"val_auc={metrics['val']['auc']:.3f} val_acc={metrics['val']['acc']:.3f}"
        )
        history.append(metrics)
        va = metrics["val"]["auc"]
        # First best val AUC wins on ties.
        if not np.isnan(va) and va > best_val:
            best_val, best_ep = va, epoch
            torch.save(model.state_dict(), fold_dir / "best.pt")

    model.load_state_dict(torch.load(fold_dir / "best.pt", map_location=device, weights_only=True))
    test_m = eval_loader(model, loaders["test"], device)
    best = history[best_ep - 1]
    print(
        f"[DSMIL fold{fold}] best_ep={best_ep} "
        f"val_auc={best['val']['auc']:.3f} test_auc={test_m['auc']:.3f} test_acc={test_m['acc']:.3f}"
    )
    return {
        "fold": fold,
        "best_epoch": best_ep,
        "best_val_auc": best["val"]["auc"],
        "best_val_acc": best["val"]["acc"],
        "test_auc_at_best_val": test_m["auc"],
        "test_acc_at_best_val": test_m["acc"],
    }


def main():
    p = argparse.ArgumentParser()
    p.add_argument("--feat-dir", type=Path, default=ROOT / "feats" / "pt_files")
    p.add_argument("--result-dir", type=Path, default=ROOT / "results")
    p.add_argument("--epochs", type=int, default=50)
    p.add_argument("--lr", type=float, default=5e-3)
    p.add_argument("--folds", default="0,1,2,3,4")
    args = p.parse_args()
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    summaries = []
    for fold in [int(x) for x in args.folds.split(",")]:
        summaries.append(
            train_fold(fold, args.feat_dir, args.result_dir, args.epochs, args.lr, device)
        )
    save_cv_summary(args.result_dir, summaries, "DSMIL")


if __name__ == "__main__":
    main()
