...
class PPEG(nn.Module):
    ...
    def forward(self, x):
        ...
        # avoid padded feature map smaller than kernel; use zero padding
        if H < 7:
            H, W = 7, 7
...
class RRTEncoder(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # no dim reduction needed
        if reduce_ratio == 0:
            pass
        # auto dim reduction by multi-scale; dim unchanged
        elif reduce_ratio == -1:
            reduce_ratio = n_layers - 1
        # fixed reduced dim (power of 2)
        else:
            pass
    ...
    def forward(self, x, no_pool=False, return_trans_attn=False, return_attn=False, no_norm=False):
        ...
        if len(x.shape) == 4:
            ...
            x = x.transpose(1, 2)
            shape_len = 4
        batch, num_patches, C = x.shape  # features directly
