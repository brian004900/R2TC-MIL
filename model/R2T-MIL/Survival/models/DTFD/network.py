...
class DimReduction(nn.Module):
    def __init__(self, n_channels, m_dim=512, dropout=False, act="relu", n_robust=0, rrt=False, rrt_convk=15, rrt_moek=3, rrt_as=False, rrt_md=False):
        ...
        # add custom re-embedding module
...
if __name__ == "__main__":
    ...
    # group=8, distill='MaxMinS' defaults on LUAD&LUSC from the paper
    # check whether the optimization block should be externalized
    dtfd = DTFD(
        x.device,
        1e-5,
        1e-5,
    ...
    # training needs labels; loss is computed and backpropagated inside
    pred = dtfd(x, y)
    print(pred.size())  # 1,C
    dtfd.eval()
    x_test = dtfd(x)
